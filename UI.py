...
class UserInterface():
    ...
    def getETA(self, jumps):
        ...
        nowReset = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=now.hour,minutes=now.minute)
        ...
        if jumps <= todayAvailableJumps:
            ...
            if alreadyQueued < 21 * 60:
                if alreadyQueued > 17 * 60:
                    #print('on to next')
                    #print(alreadyQueued)
                    alreadyQueued = alreadyQueued - 21 * 60
                eta -= datetime.timedelta(seconds=alreadyQueued)
    ...
    def settings(self):
        ...
        invite = tk.Button(self.settingsWindow,
                           text="With thanks to the Fleet Carrier Owner's Club (Especially Ed, NailoVint and Brandstaetter)",
                           font="Ebrima 13 bold",
                           fg='orange',
                           activeforeground='orange',
                           bg='#222222',
